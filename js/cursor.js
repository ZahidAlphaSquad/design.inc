...
export class GooCursor {
    ...
    constructor(DOM_el) {
        ...
        // Apply filter if not Firefox
        if (!isFirefox()) {
            this.DOM.inner.style.filter = 'url(#gooey)';
        }

        // TTL from data attribute
        this.settings.ttl = parseFloat(this.DOM.el.getAttribute('data-ttl')) || this.settings.ttl;
    ...
    layout() {
        // The number of columns is defined in a CSS variable --columns
        this.columns = parseInt(getComputedStyle(this.DOM.el).getPropertyValue('--columns'));
        // Calculate cell size
        this.cellSize = winsize.width / this.columns;
        // Calculate number of rows
        this.rows = Math.ceil(winsize.height / this.cellSize);
